...
void processCommand(const std::string& line) {
                ...
                while (std::getline(iss, cmd)) {
                    ...
                    if (cmd.find("打印 ") == 0) {
                        ...
                        if (eq_pos != std::string::npos) {
                            std::string name = trim(cmd.substr(4, eq_pos - 4));
    ...
    if (line.find("如果 ") == 0 && line.find("那么") != std::string::npos) {
        ...
        std::string cond_str = line.substr(7, then_pos - 7);
    ...
    if (line.find("重复执行 ") == 0 && line.find("次") != std::string::npos) {
        std::string num_str = trim(line.substr(13, line.find("次") - 13));
    ...
    if (line.find("设 ") == 0) {
        ...
        if (eq_pos != std::string::npos) {
            std::string name = trim(line.substr(4, eq_pos - 4));
